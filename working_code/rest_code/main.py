...
def process_entity_relations(entity_relations_str, verbose=True):
    ...
    #print(entity_relations_str)     #changed
    for s in entity_relations_str:
        if((s.split(':')[0]) == '1.000'):        #have triples of full confidence only
...
def stanford_ie(input_filename, verbose=True, generate_graphviz=False):
    ...
    df = pd.read_csv(input_filename, usecols = ['index', 'coref_content', 'industry'])
    df_output = pd.DataFrame(columns = ['industry', 'index', 's1', 'r', 's2'])
    ...
    for idx, index, content, industry in df.itertuples():
        ...
        command_str = 'echo ' + '"{}"'.format(content) + ' > Stanford-OpenIE-Python/input_filename.txt'
        #print(command_str, type(command_str))
        os.system(command_str)

        new_filename = ''
        input_filename = 'Stanford-OpenIE-Python/input_filename.txt'
        new_filename += cwd + '/{} '.format(input_filename)
        ...
        # iterate the triples and put them in submission format
        # industry_name, index, s1, r, s2

        for item in results:
            [a, b, c] = item
            #print(industry, index, a, b, c)
            df_output = df_output.append({'index': index, 'industry': industry, 's1': a, 'r': b, 's2': c}, ignore_index = True)

    
    df_output.to_csv('submission_openIE.csv')        
...
def main(args):
    ...
    stanford_ie(filename, verbose, generate_graphviz)    
