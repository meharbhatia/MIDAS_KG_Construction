...
from triplets_using_flair import entityList
...
with open(r'C:\Users\mehar\Desktop\MIDAS\Internship\data\icdm_trial.csv','rt', encoding="utf8")as mainfile:
	...
	for row in data:
		...
		entity_list = quicksort(entity_list)
		print(entity_list)


		entity_list_USE = entity_list
		for entity in entity_list:
		...
		print("BREKINGT HEM DOWN")
		entity_list = entityList(entity_list)
		...
		i = 0
		csv_list = []
		with open(r'C:\Users\mehar\Desktop\MIDAS\Internship\Mehar_Method\29July\new_19_final.csv','rt', encoding="utf8")as infile:
		...
		for rowlist in csv_list:
			...
			#Switch with attribute 1
				...
				if ((attribute1 == ent) for ent in entity_list):
					print(" ")
				if ((attribute3 == ent) for ent in entity_list):
					print(" ")
				else:
					...
					# else:
						...
						# ratios = [fuzz.ratio(attribute1, ent) for ent in entity_list]
						# best_match1 = max(entity_list, key=lambda ent: fuzz.ratio(attribute1, ent))
						# best_match1 = entity_list[ratios.index(max(ratios))]
						# print(best_match1)
						...
						# ratios = [fuzz.ratio(attribute3, ent) for ent in entity_list]
						# best_match2 = entity_list[ratios.index(max(ratios))]
						# print(best_match2)
				# Ratios = process.extract(attribute3,entity_list)
				# highest = process.extractOne(attribute3,entity_list)
				# rowlist[4] = highest[0]
...
with open("results_new_19_final_usingList_REFINED.csv", "w", newline = "", encoding = "utf8") as f:
    writer = csv.writer(f)
